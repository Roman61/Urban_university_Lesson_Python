...
async def get_buying_list(call):
    for k, v in enumerate(dietary_supplements):
        with open(dietary_supplements[v][0], "rb") as img:
            await call.message.answer_photo(img, f'Название: {v} | Описание: {dietary_supplements[v][2]} | '
                                                 f'Цена: {dietary_supplements[v][1]}>')
